...
fn test_tls_connector() -> tls_api_native_tls::TlsConnector {
    let root_ca = include_bytes!("../tls.der");
...
fn main() {
    ...
    let tls_option = httpbis::ClientTlsOption::Tls("127.0.0.1".to_owned(),
                Arc::new(test_tls_connector()));
    let addr = SocketAddr::new("::1".parse().unwrap(), port);

    let grpc_client = Arc::new(
        grpc::Client::new_expl(&addr, "127.0.0.1", tls_option, client_conf).unwrap()
    );

    let client = LightningClient::with_client(grpc_client);

    let req = GetInfoRequest::new();
